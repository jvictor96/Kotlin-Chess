...
class MeuController(private val service: MatchService) {
    ...
    @PostMapping("/{player}/resign/{id}")
    fun criar(@PathVariable player: String, @PathVariable id: Int): Board? {
        return service.resign(id, player)
    }

    @PostMapping("/{player}/move/{id}")
    fun mvoe(@PathVariable id: Int, @PathVariable player: String, @RequestBody movement: MoveRequest): Board ?{
